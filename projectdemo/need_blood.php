...
<body>
  ...
  <div id="page-container" style="margin-top:50px; position: relative;min-height: 84vh;background-color: #e0e0d1">
...
<div class="font-italic">Mobile Number<span style="color:red">*</span></div> 
    <input type="text" pattern="^\d{10}$" class="form-control" name="mobileno" aria-describedby="inputGroupPrepend" required>  
</div>
...
<div><select name="loco" class="form-control" required></div>
<option value=""selected disabled>Select</option>
    <?php
      include 'conn.php';
    ...
    while($row=mysqli_fetch_assoc($result)){
     ?>
     <option value=" <?php echo $row['loco_id'] ?>"> <?php echo $row['loco_name'] ?> </option>
  <?php } ?>
  </select>
  </div>
  </div>
...
<?php if(isset($_POST['search'])){
  ...
  $bg=$_POST['blood'];
  $lg=$_POST['loco'];
  $sql="select * from donor_details join loco,blood where donor_details.loco=loco.loco_id AND loco='{$lg}' and donor_details.donor_blood=blood.blood_id AND donor_blood='{$bg}' order by rand() limit 10";
  $result=mysqli_query($conn,$sql) or die("query unsuccessful.");
    ...
    while($row = mysqli_fetch_assoc($result)) {
      ...
      <div class="card" style="width:300px">
          ...
          <div class="card-body">
            ...
            <p class="card-text">
              <b>Donor_id : </b> <b><?php echo $row['donor_id']; ?></b><br>
              ...
              <b>Location : </b> <?php echo $row['loco_name']; ?><br>
            </p>
